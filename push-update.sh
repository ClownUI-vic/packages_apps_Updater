...
updates_dir=/data/clown_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have ClownUI-VERSION-DATE-TYPE-* as format"
...
# Assume ClownUI-VERSION-DATE-TYPE-*.zip
...
adb shell "killall org.clown.updater 2>/dev/null"
adb shell "sqlite3 /data/data/org.clown.updater/databases/updates.db" \
